    ...
    >
      ...
      <div className="flex items-start justify-between px-4 pt-4 pb-3">
        ...
        <div className="shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
          {/* Badge Queue — tournois actifs en mode flottant */}
          {status === 'active' && tournament.tournament_config?.floatingSchedule && (
            <button
              onClick={(e) => {
                e.stopPropagation()
                navigate(`/tournament/${tournament.id}/queue`)
              }}
              title="File d'attente"
              className="h-6 px-2 rounded-md text-[11px] font-bold
                bg-padel-blue/10 text-padel-blue hover:bg-padel-blue/20 transition-colors"
            >
              Queue
            </button>
          )}
